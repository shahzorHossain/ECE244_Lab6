...
void addPoly(stringstream& linestream, ShapeArray& shapeArray) {
   ...
   while (!linestream.fail()) {
      ...
      xpoints[nPoint] = vertices[nPoint].x;
      ypoints[nPoint] = vertices[nPoint].y;
      if (!linestream.fail ()) 
         nPoint++;
   }
   ...
   for(int i =0; i < nPoint; i++){
      xpoints[i] = vertices[i].x - xcen;
      ypoints[i] = vertices[i].y - ycen;
   }
   if (nPoint < 3) {
   ...
   // shapeArray database.
   Polygon* poly = new Polygon(name, colour, xcen, ycen, xpoints, ypoints, nPoint);
